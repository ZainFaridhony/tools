...
export const Dashboard: React.FC<DashboardProps> = ({ onSelectTool }) => {
  ...
  return (
    <div className="flex flex-col p-4 pb-20 relative">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center w-full md:w-auto">
          ...
          <div className="relative w-full md:w-64">
            <input
              ...
              className="w-full border-2 border-black px-3 py-1 pl-9 font-mono text-xs md:text-sm focus:outline-none focus:bg-retro-yellow/20 transition-colors"
            />
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="font-mono font-bold mr-2 py-1 dark:text-white text-xs md:text-sm hidden md:inline">FILTER:</span>
            {categories.map(cat => (
              <button
                key={cat}
      ...
      {/* Content */}
      {sortOrder === 'default' && selectedCategory === 'All' && !searchQuery ? (
          ...
          {(selectedCategory !== 'All' || searchQuery) && (
            <div className="mb-6 flex items-center gap-4">
              <h2 className="font-display text-2xl inline-block border-b-4 border-retro-hotPink dark:text-white">
                {searchQuery ? `SEARCH: "${searchQuery.toUpperCase()}"` : selectedCategory.toUpperCase()}
              </h2>
              {searchQuery && (
